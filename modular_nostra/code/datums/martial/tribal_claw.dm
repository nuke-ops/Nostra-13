...
/datum/martial_art/tribal_claw/proc/faceScratch(mob/living/carbon/human/A, mob/living/carbon/human/D)
	...
	D.apply_damage(17, BRUTE, BODY_ZONE_HEAD, def_check)
	D.confused += 5
	D.blur_eyes(5)
...
/*
Claw Stomp, can only be done if the target is in crit, being held in a tier 3 grab by the user or if they are sleeping.
Deals 10 brute to chest(reduced by armor) and causes massive stamina damage.
*/
/datum/martial_art/tribal_claw/proc/jugularCut(mob/living/carbon/human/A, mob/living/carbon/human/D)
	...
	if((D.health <= D.crit_threshold || (A.pulling == D && A.grab_state >= GRAB_NECK) || D.IsSleeping()))
		log_combat(A, D, "claw stomp (Tribal Claw)")
		D.visible_message("<span class='warning'>[A] slams there clawed foot onto [D]'s chest!</span>", \
							"<span class='userdanger'>[A] stomps your chest!</span>")
		D.apply_damage(10, BRUTE, BODY_ZONE_HEAD, def_check)
		//D.bleed_rate = clamp(D.bleed_rate + 20, 0, 30)
		D.apply_damage (200, STAMINA, BODY_ZONE_CHEST, def_check)
...
/mob/living/carbon/human/proc/tribal_claw_help()
	...
	to_chat(usr, "<span class='notice'>Claw Stomp</span>: Harm Harm Harm Grab. Causes your target to take immense stamina damage, works only if you grab your target by their neck, if they are sleeping, or in critical condition.")
