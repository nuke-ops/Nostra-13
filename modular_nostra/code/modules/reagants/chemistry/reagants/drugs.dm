...
/datum/reagent/drug/kronkaine/on_mob_life(mob/living/carbon/kronkaine_fiend, delta_time, times_fired)
	...
	kronkaine_fiend.adjustOrganLoss(ORGAN_SLOT_HEART, 0.4 * delta_time)
	kronkaine_fiend.Jitter(10 * delta_time)
	kronkaine_fiend.AdjustSleeping(-20 * delta_time)
	kronkaine_fiend.drowsyness = max(kronkaine_fiend.drowsyness - (5 * delta_time), 0)
	if(volume < 10)
		return
...
/datum/reagent/drug/kronkaine/overdose_process(mob/living/kronkaine_fiend, delta_time, times_fired)
	. = ..()
	kronkaine_fiend.adjustOrganLoss(ORGAN_SLOT_HEART, 1 * delta_time)
	kronkaine_fiend.Jitter(10 * delta_time)
